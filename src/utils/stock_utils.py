...
class BaseCalculate(metaclass=abc.ABCMeta):
    ...
    def __init__(self, save_dir=get_default_output_path(), *args, **kwargs):
...
class CalHighLowPoint(BaseCalculate):
    ...
    def caculate_turning_point(self, pre_df, start_x, y_col, x_col, mark):
        ...
        """
        df = pre_df.loc[
            pre_df[x_col] < start_x - 3
        ].copy()  # 如果上一个点是最高/低点则次高点最少距离3天
        if mark:
            # 峰值
            ...
            df_p = df.loc[
                (df["p3"] == 1) & (df["p2"].shift(-2) == 1) & (df["p2"].shift(-3) == 1),
                "p4",
            ] = 1
    ...
    def circle_find(self, start_mark, i_start, pre_df, py_col, ly_col, x_col) -> List:
        ...
        while True:
            ...
            if start_mark:
                # 峰值
                res_one = self.caculate_turning_point(
                    pre_df, i, py_col, x_col, start_mark
                )
            else:
                # 谷值
                res_one = self.caculate_turning_point(
                    pre_df, i, ly_col, x_col, start_mark
                )
            if not res_one[0]:
                break
...
def calc_start_end_date(n_folds, start=None, end=None):
    ...
    if end is None:
        # 没有end也没start，end＝today，否则使用n_folds计算end
            ...
            today
            if start is None
            else ABuDateUtil.begin_date(-365 * n_folds, date_str=start, fix=False)
        )
    # int类型的end, today转换
    ...
    **kwargs,
    ...
    if api_type == "qstock":
        ...
        if not code.startswith(("60", "0")):
            ...
            df = TXApi().kline(
                symbol=code, n_folds=n_folds, start=start_date, end=end_date
            )
            if df is None:
                return df
        ...
        df = BaoStockExecutor().get_history_time_data(
            code=code, start_date=start_date, end_date=end_date, **kwargs
        )
        return df
...
def cal_exist_zt_in_windows(df: pd.DataFrame, windows=10):
    ...
    df["change"] = (
        df["close"]
        .rolling(window=2)
        .apply(lambda x: round((float(x[-1]) - float(x.iloc[0])) / float(x.iloc[0]), 4))
    )
...
if __name__ == "__main__":
    res = get_day_k_data_multi(
        start_date="2024-05-07", end_date="2024-07-30", codes=["600460"]
    )
